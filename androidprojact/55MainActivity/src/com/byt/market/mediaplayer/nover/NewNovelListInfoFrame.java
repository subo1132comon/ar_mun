 ...
 * 列表分类详情
 */
public class NewNovelListInfoFrame extends BaseActivity implements
	...
	public ProgressBar playbar_progress,radio_progress;
	public IPlayback service;
	//public NewMusicListFragment cateListFragment;
	public NewNovelListFragment cateListFragment;
	public boolean radio_playing = false;
	public boolean is_radio = false;
	public String m_name = "  loading---";
		...
		public void onReceive(Context context, Intent intent) {
			try {
				if (PlayMusicService.PREPARED_PLAY_MUSIC_ACTION.equals(intent
					...
					playbarlayout.setVisibility(View.VISIBLE);
					is_radio = true;
					radio_progress.setVisibility(View.VISIBLE);
					playBarNextButton.setVisibility(View.GONE);
					musicName.setText(m_name);
				}else if(RediaoService.IS_PLAYING_ACTION.equals(intent.getAction())){
					m_name = intent.getStringExtra("name");
					refreshPlayStateIcon2();
					sendBroadcast(new Intent(PlayMusicService.NOTIF_CANCAL_ACTION));
					radio_playing = true;
					radio_progress.setVisibility(View.GONE);
					SharedPreferences yuVer = MyApplication.getInstance()
							.getSharedPreferences("radioplay", 0);
					SharedPreferences.Editor editor = yuVer.edit();
					editor.putBoolean("isplay",true);
					editor.putString("musicname",m_name);
					editor.commit();
				}else if(RediaoService.NOTIF_STOP_ACTION.equals(intent.getAction())){
					setClose();
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	private void initReceiver() {
		...
		filter.addAction(RediaoService.IS_PLAYING_ACTION);
		filter.addAction(RediaoService.NOTIF_STOP_ACTION);
...
//				}
				...
				cateListFragment = new NewNovelListFragment();
				bundle.putString("hot", Constants.CATE_HOT);
	...
	public void initView() {
		...
		radio_progress = (ProgressBar) findViewById(R.id.play_ProgressBar);
	...
	public void onClick(View v) {
		...
		case R.id.playbutton:// 播放按钮
			try {
				SharedPreferences yuVer = MyApplication.getInstance()
						.getSharedPreferences("radioplay", 0);
				if(!yuVer.getBoolean("isplay", false)){
					if (service != null&&!is_radio) {
	...
	protected void onResume() {
		...
		if(yuVer.getBoolean("isplay", false)){
			...
			playBarButton
			.setImageResource(R.drawable.playbar_btn_pause);
			musicName.setText(yuVer.getString("musicname", ""));
			playBarNextButton.setVisibility(View.GONE);
		}else {
			try {
				handler.removeMessages(0);
	...
	private void refreshPlayStateIcon2(){
		playBarButton
		.setImageResource(R.drawable.playbar_btn_pause);
		musicName.setText(m_name);
	}
