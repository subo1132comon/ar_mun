 ...
 * 总排行
 */
public class RankTotalFragment extends ListViewFragment {
	...
	protected String getRequestUrl() {
		return Constants.LIST_URL + "?qt=" + Constants.TOTAL_RANK + "&pi="
		...
		public void bindView(int position, BigItem item, BigHolder holder) {
			...
			case BigItem.Type.LAYOUT_RANKLIST:
				...
				for (final AppItem appItem : item.rankItems) {
					...
					itemHolder.tv_rank_number.setVisibility(View.GONE);
					itemHolder.tv_rank_other_number.setVisibility(View.GONE);
					...
					if(appItem.isshare==1){
						...
						itemHolder.downBtn2.setOnClickListener(new OnClickListener() {
							...
							public void onClick(View arg0) {
								...
								if(itemHolder.downBtn2.getText().toString().equals(RankTotalFragment.this.getActivity().getString(R.string.sharedown)))
								{
									Intent intent =new Intent();
									intent.setClass(RankTotalFragment.this.getActivity(),ShareActivity.class);						
									intent.putExtra("sendstring", appItem); 
									RankTotalFragment.this.getActivity().startActivity(intent);
								}
								else
								{
						...
						if(!imageLoader.getPause().get()){
							Drawable drawable=RankTotalFragment.this.getActivity().getResources().getDrawable(R.drawable.progress_listitem);
