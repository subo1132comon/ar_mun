 ...
 * 列表分类详情
 */
public class NewTVListInfoFrame extends BaseActivity implements
	...
	public NewTVListFragment cateListFragment;
			...
			try {
				if (PlayMusicService.PREPARED_PLAY_MUSIC_ACTION.equals(intent
					...
					cateListFragment.getAdapter().notifyDataSetChanged();
						...
						.getAction())) {
					refreshPlayStateIcon();
				}else if(Constants.SHARE_ACTION.equals(intent.getAction())){
					cateListFragment.playVdiao((RingItem) intent.getParcelableExtra("share"));
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
...
//				}
				...
				cateListFragment = new NewTVListFragment();
				bundle.putString("hot", Constants.CATE_HOT);
