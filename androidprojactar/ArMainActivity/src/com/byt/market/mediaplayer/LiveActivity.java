					...
					.setOrderId(orderId), new PayssionResponseHandler() {
						...
						public void onSuccess(PayssionResponse response) {
							if (response.isSuccess()) {
								...
								if (null != detail) {
									Toast.makeText(LiveActivity.this, detail.getStateStr(), Toast.LENGTH_SHORT).show();
									if (PPaymentState.COMPLETED == detail.getState()) {
										//the payment was paid successfully
										LiveActivity.this.sendBroadcast(new Intent("COM.AR.PAY"));
								...
								Toast.makeText(LiveActivity.this, response.getDescription(), Toast.LENGTH_SHORT).show();
