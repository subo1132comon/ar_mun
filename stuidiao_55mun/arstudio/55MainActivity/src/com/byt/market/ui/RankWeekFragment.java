 ...
 * 周排行
 */
public class RankWeekFragment extends ListViewFragment {
	/* add by zengxiao */
	...
	protected String getRequestUrl() {
		return Constants.LIST_URL + "?qt=" + Constants.GAME_RANK + "&pi="
	...
	private class RankAdapter extends ImageAdapter {
		...
		public View newView(LayoutInflater inflater, BigItem item) {
			...
			case BigItem.Type.LAYOUT_LOADING:
				LogUtil.e("cexo",
						"RankWeekFragment newView BigItem.Type.LAYOUT_LOADING:");
		...
		public void bindView(int position, BigItem item, BigHolder holder) {
					...
					/* add by zengxiao for change UI */
						if(appItem.isshare==1){
							...
							public void onClick(View arg0) {
								...
								if(itemHolder.downBtn2.getText().toString().equals(RankWeekFragment.this.getActivity().getString(R.string.sharedown)))
								{
									Intent intent =new Intent();
									intent.setClass(RankWeekFragment.this.getActivity(),ShareActivity.class);						
									intent.putExtra("sendstring", appItem); 
									RankWeekFragment.this.getActivity().startActivity(intent);
								}
								else
								{
							...
							if(!imageLoader.getPause().get()){
								Drawable drawable=RankWeekFragment.this.getActivity().getResources().getDrawable(R.drawable.progress_listitem);
			...
			case BigItem.Type.LAYOUT_LOADING:
				LogUtil.e("cexo",
						"RankWeekFragment bindView BigItem.Type.LAYOUT_LOADING : "
								+ +position);
				break;
			case BigItem.Type.LAYOUT_LOADFAILED:
